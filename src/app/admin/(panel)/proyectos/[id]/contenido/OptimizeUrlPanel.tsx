...
import { Loader2, Search, AlertCircle, FileText, History, Wand2 } from "lucide-react";
...
export default function OptimizeUrlPanel({ projectId }: { projectId: string }) {
  ...
  function loadHistory(forUrl: string) {
    if (!forUrl.trim()) return;
    setLoadingHistory(true);
    fetch(`/api/proyectos/${projectId}/contenido/optimizar?url=${encodeURIComponent(forUrl.trim())}`)
      .then((r) => r.json())
      .then((d: OptimizationRun[]) => Array.isArray(d) && setHistory(d))
